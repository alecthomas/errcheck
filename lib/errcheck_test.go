...
func TestUnchecked(t *testing.T) {
	err := CheckPackage("github.com/kisielk/errcheck/example", make(map[string]*regexp.Regexp), false, true)
...
func TestBlank(t *testing.T) {
	err := CheckPackage("github.com/kisielk/errcheck/example", make(map[string]*regexp.Regexp), true, true)
	uerr, ok := err.(UncheckedErrors)
	if !ok {
		t.Fatal("wrong error type returned")
	}
